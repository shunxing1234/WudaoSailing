...
from nets.BertTransformer import TransformerLayer
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, emb, seg):
        ...
        if self.mask == "fully_visible":
            '''
            Change of dimensions of mask matrix
            (seg > 0): [batch_size x seq_length]             
            unsqueeze(1): [batch_size x 1 x seq_length] 
            repeat(1, seq_length, 1): [batch_size x seq_length x seq_length] 
            unsqueeze(1): [batch_size x 1 x seq_length x seq_length]
            '''
            mask = (seg > 0). \
                unsqueeze(1). \
            ...
            # Mask upright triangle of the input embedding matrix
