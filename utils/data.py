...
class BertDataset(Dataset):
    ...
    def worker(self, proc_id, start, end):
        ...
        set_seed(self.seed)
        docs_buffer = []   #存储多份document，到了docs_buffer_size的时候用self.build_instances得到instances然后保存起来
        document = []      #存储一份document里面的sentences
        pos = 0
        dataset_writer = open("dataset-tmp-" + str(proc_id) + ".pt", "wb")
        text_writer = open("rawtext-tmp-" + str(proc_id) + ".pt", "wb")
        with open(self.corpus_path, mode="r", encoding="utf-8") as f:
            #文件的第一行似乎是heading,所以不用读
            while pos < start:
                f.readline()
                pos += 1
            while True:
                ...
                if pos >= end:
                    if len(docs_buffer) > 0:
                        ...
                        for instance in instances:
                            raw_text = self.tokenizer.convert_ids_to_tokens(instance[0])
                            pickle.dump(raw_text, text_writer)
                            pickle.dump(instance, dataset_writer)
                    break

                if not line.strip():       #意味着到了文件结束的时候
                    if len(document) >= 1:  #只要document有内容就要添加
                        docs_buffer.append(document)
                    document = []           #这一份文件已读完，清空document
                    if len(docs_buffer) == self.docs_buffer_size:   #docs_buffer到达容量设置，开始生成instances并存储
                        ...
                        docs_buffer = []
                    continue
                #没结束的时候自然就一点一点加sentence，注意是tokenize以后的
    ...
    def create_ins_from_doc(self, all_documents, document_index):
        ...
        #-3应该是因为还要减去一个CLS两个SEP，具体看后面src的结构
        max_num_tokens = self.seq_length - 3   #暂定seq的长度
        target_seq_length = max_num_tokens     #最终seq的长度（有可能要缩短）
        if random.random() < self.short_seq_prob:   #在short_seq_prob的概率下
            target_seq_length = random.randint(2, max_num_tokens)   #target_seq_length从2到max_num_tokens间选择
        instances = []
        current_chunk = []
        current_length = 0
        i = 0
        while i < len(document):
            segment = document[i]                  #当前sentence
            current_chunk.append(segment)
            current_length += len(segment)
            if i == len(document) - 1 or current_length >= target_seq_length:   #每一个batch的数据
                if current_chunk:
                    a_end = 1
    ...
    def __iter__(self):
        while True:
            ...
            for ins in instances:
                if len(ins) == 4:    #非dynamic masking的情况
                    #ins = [src,tgt_mlm,is_random,seg_pos]
                    src.append(ins[0])
                    ...
                    #seg大概长这样 1,1,1,1,2,2,2,2,2,0,0,0，  1是之前第一部分的长度，2是第二部分的长度，0是剩下的长度
                    seg.append([1] * ins[3][0] + [2] * (ins[3][1] - ins[3][0]) + [0] * (len(ins[0]) - ins[3][1]))
                else:                #dynamic masking的情况
                    ...
                    masked_words_num += len(tgt_mlm_single)      #更新mask_word_num
                    src.append(src_single)                        #更新src
                    tgt_mlm.append([0] * len(ins[0]))
                    for mask in tgt_mlm_single:                  #建立mask（非mask是0）
                        tgt_mlm[-1][mask[0]] = mask[1]
    ...
    def create_ins_from_doc(self, document):
        ...
        while i < len(document):
            ...
            if i == len(document) - 1 or current_length >= target_seq_length:
                if current_chunk:
                    #将current_chunk随机从中间分开，再随机安排这两块的顺序
                    a_end = 1
                    if len(current_chunk) >= 2:
                        a_end = random.randint(1, len(current_chunk) - 1)    #current_chunk里的一个随机位置
                    ...
                    # construct src: 就是tokenize后的句子的内容，包括之前的两个部分，以
                    # construct seg_pos, 就是第一部分和第二部分结束的位置
                    src = []
                    src.append(self.vocab.get(CLS_TOKEN))
                    src.extend(tokens_a)
                    src.append(self.vocab.get(SEP_TOKEN))
...
class GsgDataset(BertDataset):
    ...
    def create_ins_from_doc(self, all_documents, document_index):
        ...
        print(sentence_selection_strategy, mask_seq_list)
        while i < len(document):
            segment = document[i]
