...
from dataprocessing.tokenization import make_tokenizer
...
def make_data_loader(dataset, tokenizer, batch_size, num_iters, args, shuffle=False, block_collate=False):
    ...
    if args.transformer_xl:
        batch_sampler = dataprocessing.samplers.DistributedSequentialSampler(len(dataset),
                                                                         num_iters,
                                                                         batch_size,
                                                                         rank,
                                                                         world_size)
        ...
        if shuffle:
            sampler = dataprocessing.samplers.RandomSampler(dataset, replacement=True,
        ...
        if distributed:
            batch_sampler = dataprocessing.samplers.DistributedBatchSampler(sampler, batch_size, drop_last, rank,
                                                                        world_size,
...
def make_tfrecord_loaders(args):
    ...
    train = dataprocessing.tf_dl.TFRecordDataLoader(args.train_data,
                                                **data_set_args)
    ...
    if args.valid_data is not None:
        valid = dataprocessing.tf_dl.TFRecordDataLoader(args.valid_data,
                                                    **data_set_args)
    test = None
    if args.test_data is not None:
        test = dataprocessing.tf_dl.TFRecordDataLoader(args.test_data,
                                                   **data_set_args)
    tokenizer = dataprocessing.make_tokenizer(args.tokenizer_type,
                                          train,
                                          args.tokenizer_path,
                                          args.vocab_size,
...
def make_loaders(args, tokenizer):
    ...
    if args.train_data is not None:
        train = dataprocessing.make_dataset(**data_set_args)
        if dataprocessing.should_split(split):
    ...
    if valid is None and args.valid_data is not None:
        ...
        valid = dataprocessing.make_dataset(**eval_set_args)
        eval_set_args['tokenizer'] = tokenizer
    if test is None and args.test_data is not None:
        eval_set_args['path'] = args.test_data
        test = dataprocessing.make_dataset(**eval_set_args)
...
def configure_data():
    ...
    # These are options that are used by dataprocessing, but are either
