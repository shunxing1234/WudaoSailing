...
import mpu
import pandas as pd
from setup_model import get_model
    ...
    with torch.no_grad(), open(output_path, "w") as output:
        ...
        #        print('-----------------------------------------------------------')
            while count < 1:
                count += 1
                line = re.sub('小布','',line)
                raw_text = '问题:' + line.strip() + '?' + '答案:[gMASK]'
                
                torch.distributed.barrier(group=mpu.get_model_parallel_group())

                terminate_runs, raw_text, context_tokens_tensor, context_length = read_context(raw_text, tokenizer, args, output)
                if terminate_runs == 1:
                    return
