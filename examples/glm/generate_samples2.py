...
def sample_sequence(model, tokenizer, context_tokens, context_length, args, device, mems=None, end_tokens=None):
    ...
    # print('---------------------------------------')
    while counter < 128:#args.out_seq_length:
...
def read_context(text, tokenizer, args, output):
...
def generate_samples(model, tokenizer, args, device):
    ...
    output_path = output_path+ args.port+".txt"
    with torch.no_grad(), open(output_path, "w") as output:
        # while True:
        f = pd.read_csv('./query_oppo.csv')
        ques=f['query']
        idx=np.int(args.port)-30000
        print(idx)
        n = 0
        for line in ques[(idx-1)*3000:3000*idx]:
            count = 0
            n+=1
            if n % 50 == 0:
                print('n:'+str(n))
            ...
            while count < 10:
                count += 1
                line = re.sub('小布','',line)
                raw_text0 = '问题：' + line.strip() + '?' + '回答：[gMASK]'
                torch.distributed.barrier(group=mpu.get_model_parallel_group())

                terminate_runs, raw_text, context_tokens_tensor, context_length = read_context(raw_text0, tokenizer, args, output)
                if terminate_runs == 1:
                    return
